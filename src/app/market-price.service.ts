...
export class MarketPriceService {
  ...
  getPriceObj(priceType: 'YDayAvgPrice' | 'RecentPrice' | 'CurrentMinPrice') {
    ...
    const obj = {
      ...
      상급아비도스: getPrice('상급 아비도스 융화 재료'),
      명돌: getPrice('명예의 돌파석'),
      위명돌: getPrice('위대한 명예의 돌파석'),
      경명돌: getPrice('경이로운 명예의 돌파석'),
      찬명돌: getPrice('찬란한 명예의 돌파석'),
      운돌: getPrice('운명의 돌파석'),
      위운돌: getPrice('위대한 운명의 돌파석'),
      수결: getPrice('수호석 결정') / 100,
      파결: getPrice('파괴석 결정') / 100,
      수호강석: getPrice('수호강석') / 100,
      파괴강석: getPrice('파괴강석') / 100,
      정제된수호강석: getPrice('정제된 수호강석') / 100,
      정제된파괴강석: getPrice('정제된 파괴강석') / 100,
      운명의수호석: getPrice('운명의 수호석') / 100,
      운명의파괴석: getPrice('운명의 파괴석') / 100,
      운명의수호석결정: getPrice('운명의 수호석 결정') / 100,
      운명의파괴석결정: getPrice('운명의 파괴석 결정') / 100,
      은총: getPrice('태양의 은총'),
      축복: getPrice('태양의 축복'),
      가호: getPrice('태양의 가호'),
      빙하: getPrice('빙하의 숨결'),
